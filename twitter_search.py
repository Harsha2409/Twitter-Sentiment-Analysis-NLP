...
def tw_search_json(query, cnt=5):
    ...
    for tweet in tweepy.Cursor(api.search, q=query, count=cnt).items():
        ...
        dTwt['authorloc'] = tweet.user.location        #author/user location
        ...
        #dTwt['coords']    = tweet.coordinates.coordinates 
        #dTwt['lat'] = tweet.place.bounding_box.coordinates[0][0][1]
        #dTwt['long'] = tweet.place.bounding_box.coordinates[0][0][0]
        data.append(dTwt)
    results['meta'] = meta
    results['data'] = data
    return results
...
def tw_search(api):
    ...
    csvWriter.writerow(["created", "text", "retwc", "hashtag", "followers", "friends", "username", "usersince", "authorloc", "source", "latitude", "longitude"])
    ...
    for tweet in tweepy.Cursor(api.search,
        ...
        #TWEET INFO
        created = status.created_at   #tweet created
        text    = status.text         #tweet text
        tweet_id = tweet.id          #tweet ID# (not author ID#)
        #coords   = tweet.coordinates.coordinates  #geographic co-ordinates
